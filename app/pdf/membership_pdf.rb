class MembershipPdf < Prawn::Document
  ...
  TEMPLATES = {
                :header => File.join( Rails.root, 'private', 'pna-membership-header.png' ),
                :card => {
                  :charter => File.join( Rails.root, 'private', "pna-charter-member-card.png"),
                  :standard => File.join( Rails.root, 'private', "pna-member-card.png")
                }
              }
  MARGIN = -0.5.in
  ...
  def to_pdf( membership )
    ...
    card = membership.member.is_charter_member? ? TEMPLATES[ :card ][ :charter ] : TEMPLATES[ :card ][ :standard ]

    image card, :at => [ 0.86.in, PAGE_SIZE[1] - 8.17.in ], :fit => [ PAGE_SIZE[ 0 ] - 0.86.in * 2, PAGE_SIZE[ 1 ] - 1.14.in ]
